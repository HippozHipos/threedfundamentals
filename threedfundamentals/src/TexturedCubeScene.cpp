...
TexcuredCubeScene::TexcuredCubeScene(olc::PixelGameEngine* pge, const olc::Sprite* const texture) :
	texture{ texture },
	pubeScreenTransformer{ pge } 
{
}

void TexcuredCubeScene::Update(const olc::PixelGameEngine* const pge)
{
...
void TexcuredCubeScene::Draw(olc::PixelGameEngine* pge) const
{
	pge->Clear(olc::BLACK);
	...
	for (size_t i = 0; i < triangles.indices.size() / 3; i++)
	{
		...
		{
			TriangleRaster::DrawTexturedTriangle(pge,
